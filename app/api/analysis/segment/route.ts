...
import { generateSegmentAnalysis, OfferForAnalysis } from '@/src/llm/analysis';
...
export async function POST(req: Request) {
  try {
    ...
    const segment = await db.sMBSegment.findUnique({ where: { id: segmentId } });
    if (!segment) {
    ...
    const analysis = await generateSegmentAnalysis(config, segment, transformedOffers);

    await storeInsight({
      userId: session.user.id,
      insightType: 'SEGMENT_ANALYSIS',
      title: `Segment Analysis: ${segment.name}`,
      content: analysis,
    ...
    console.error('Error generating segment analysis:', error);
